...
lists_t_strs *lyrics_lines_get(void) { return raw_lyrics; }
...
void lyrics_use_formatter(lyrics_t_formatter formatter, lyrics_t_reaper reaper,
                          void *data)
{
  if (formatter_reaper)
  {
...
lists_t_strs *lyrics_format(int height, int width)
{
  ...
  if (raw_lyrics)
  {
    result = lyrics_formatter(raw_lyrics, height, width - 1, formatter_data);
    if (!result)
    {
