...
void io_curl_cleanup() { curl_global_cleanup(); }

static size_t write_cb(void *data, size_t size, size_t nmemb, void *stream)
{
...
static size_t header_cb(void *data, size_t size, size_t nmemb, void *stream)
{
  struct io_stream *s = (struct io_stream *)stream;
  char *header;
  size_t header_size;
  ...
  else if (!strncasecmp(header, "icy-metaint:", sizeof("icy-metaint:") - 1))
  {
    char *end;
    char *value = strchr(header, ':') + 1;
...
static int debug_cb(CURL *unused1 ATTR_UNUSED, curl_infotype i, char *msg,
                    size_t size, void *unused2 ATTR_UNUSED)
{
  int ix;
  char *log;
  const char *type;
...
static int check_curl_stream(struct io_stream *s)
{
  ...
  while ((msg = curl_multi_info_read(s->curl.multi_handle, &msg_queue_num)))
  {
...
void io_curl_open(struct io_stream *s, const char *url)
{
  ...
  curl_easy_setopt(s->curl.handle, CURLOPT_HTTPHEADER, s->curl.http_headers);
  ...
  if ((s->curl.multi_status = curl_multi_add_handle(
           s->curl.multi_handle, s->curl.handle)) != CURLM_OK)
  {
...
static int curl_read_internal(struct io_stream *s)
{
  ...
  if (s->curl.need_perform_loop)
  {
    ...
    do
    {
      s->curl.multi_status = curl_multi_perform(s->curl.multi_handle, &running);
          ...
          s->curl.multi_status == CURLM_OK))
  {
    if (s->curl.multi_status != CURLM_CALL_MULTI_PERFORM)
    {
      ...
      s->curl.multi_status = curl_multi_fdset(s->curl.multi_handle, &read_fds,
                                              &write_fds, &exc_fds, &max_fd);
      ...
      ret =
          pselect(max_fd + 1, &read_fds, &write_fds, &exc_fds, &timeout, NULL);
...
static size_t read_from_buffer(struct io_stream *s, char *buf, size_t count)
{
  if (s->curl.buf_fill)
  {
    ...
    if (s->curl.buf_fill)
    {
      memmove(s->curl.buf, s->curl.buf + to_copy, s->curl.buf_fill);
      s->curl.buf = (char *)xrealloc(s->curl.buf, s->curl.buf_fill);
    }
    else
    {
...
static int read_icy_metadata(struct io_stream *s)
{
  ...
  if (read_from_buffer(s, (char *)&size_packet, sizeof(size_packet)) == 0)
  {
  ...
  while (s->curl.buf_fill < size && s->curl.handle && !s->stop_read_thread)
  {
...
ssize_t io_curl_read(struct io_stream *s, char *buf, size_t count)
{
  ...
  do
  {
    ...
    if (s->curl.icy_meta_int && s->curl.icy_meta_count == s->curl.icy_meta_int)
    {
      s->curl.icy_meta_count = 0;
    ...
    {
      to_read =
          MIN(count - nread, s->curl.icy_meta_int - s->curl.icy_meta_count);
    }
    else
    {
      to_read = count - nread;
